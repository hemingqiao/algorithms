package blogandquestion.algorithms.hashtable.displaytable1218;
 ...
 * founded at 2021/07/06 20:22:52
 * description:
...
public class Solution {
    public List<List<String>> displayTable(List<List<String>> orders) {
        ...
        for (List<String> o : orders) {
            ...
            HashMap<String, Integer> t = map.get(table);
            String key = o.get(2);
            dishes.add(key);
            t.put(key, t.getOrDefault(key, 0) + 1);
        }
        // 添加表头
        List<String> header = new ArrayList<>();
        header.add("Table");
        for (String k : dishes) header.add(k);
        res.add(header);
        // 填充表格数据
        for (Map.Entry<Integer, HashMap<String, Integer>> entry : map.entrySet()) {
            List<String> l = new ArrayList<>();
            l.add(String.valueOf(entry.getKey()));
            HashMap<String, Integer> m = entry.getValue();
            for (String k : dishes) {
                if (m.containsKey(k)) l.add(String.valueOf(m.get(k)));
                else l.add("0");
            }
            res.add(l);
        }
