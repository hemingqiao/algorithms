 ...
 * 二刷
 *
 * 原二维数组就像是一个迷宫，可以向上下左右四个方向行走。求解时可以以二维数组的每个元素作为起点，与给定字符串做匹配。又因为题目要求一个位置
 * 只能访问一次，所以如果二维数组的当前字符和目标字符匹配，将当前字符改为'.'，防止被再次访问，然后对当前字符的上下左右四个方向递归函数，只要
 * 有一个方向返回了true，就表示可以找到目标字符串，同时在递归调用结束后要把字符改为原来的字符（回溯）
 */
